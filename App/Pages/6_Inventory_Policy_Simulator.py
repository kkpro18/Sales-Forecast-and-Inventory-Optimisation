...
inventorySimulator = inventory_policy_simulator_model.InventorySimulator(env, **input_details)

if st.button("Begin Simulation"):
    env.process(inventory_policy_simulator_model.run_store(env, inventorySimulator))
    env.run(until=30)
    ...
    st.write(f"\nTotal Overstocking Cost: {inventory_policy_simulator_model.total_overstocking_cost}")
    st.write(f"\nTotal Understocking Cost: {inventory_policy_simulator_model.total_understocking_cost}")
    st.write(f"Total Restock Cost: {inventory_policy_simulator_model.total_restock_cost}")
    st.write(f"Total Cost: {inventory_policy_simulator_model.total_overstocking_cost + inventory_policy_simulator_model.total_understocking_cost + inventory_policy_simulator_model.total_restock_cost}")
    st.write("Simulation Complete: Please check logs above for details.")

if st.button("Reset Simulation"):
    inventory_policy_simulator_model.reset_simulation()
