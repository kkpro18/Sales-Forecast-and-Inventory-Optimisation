...
def column_selector(session_state_variable,label, columns):
    if session_state_variable not in st.session_state:
        st.session_state[session_state_variable] = 0
    selected_column = st.selectbox(label, options=columns,
                               index=st.session_state[session_state_variable])
    st.session_state[session_state_variable] = selected_column
    st.session_state[session_state_variable] = columns.index(selected_column)
...
if 'uploaded_dataset' in st.session_state:
    ...
    selected_date_column = column_selector('selected_date_column', "Select the Column for Invoice Date", columns)
    columns.remove(selected_date_column)
    selected_product_column = column_selector('selected_product_column', "Select the Column for Product ID / Name", columns)
    columns.remove(selected_product_column)
    selected_price_column = column_selector('selected_price_column', "Select the Column for Unit Price", columns)
    columns.remove(selected_price_column)
    selected_units_sold_column = column_selector('selected_units_sold_column', "Select the Column for Units Sold", columns)
    ...
    # product_column = st.selectbox("Select the Column for Product ID / Name", options=columns.remove(date_column), index=st.session_state.selected_product_column)
    ...
    # unit_price_column = st.selectbox("Select the Column for Unit Price", options=columns.remove(product_column), index=st.session_state.selected_price_column)
    ...
    # units_sold_column = st.selectbox("Select the Column for Units Sold", options=columns.remove(unit_price_column), index=st.session_state.selected_units_sold_column)
